...
const applyCircularLayout = (element, radius) => {
  ...
  let centerX =
    bounds.x +
    bounds.width / 2 -
    parentElementBounds.left -
    element.getBoundingClientRect().width / 2;
  let centerY =
    bounds.top +
    bounds.height / 2 -
    parentElementBounds.top -
...
let circularElements = document.querySelectorAll(
  ".inner-circle.circular-layout-element"
);
let innerRadius =
  $(".circular-layout-center-element")[0].getBoundingClientRect().width / 2;
...
const projectModalLinkContainer = document.querySelector(
...
projectCards.forEach((card) =>
  card.addEventListener("click", () => {
    for (let child of card.children) {
      ...
      if (child.className.includes("card-header"))
        projectModalHeader.innerHTML = child.innerHTML;
      if (child.className.includes("card-body"))
        projectModalContent.innerHTML = child.innerHTML;
      let abridged = document.querySelector(
      ...
      if (child.className.includes("card-footer")) {
        ...
        for (let link of child.children) {
          if (link.className.includes("project-link"))
            projectModalImgContainer.href = link.getAttribute("href");
        }
      }
    }
...
document
  ...
  .addEventListener("click", () => projectModal.close());
...
projectModal.addEventListener("click", (e) => {
  ...
  if (
    !(
      modalBounds.top < e.clientY &&
      modalBounds.bottom > e.clientY &&
      modalBounds.left < e.clientX &&
      modalBounds.right > e.clientX
    )
  ) {
    projectModal.close();
  }
